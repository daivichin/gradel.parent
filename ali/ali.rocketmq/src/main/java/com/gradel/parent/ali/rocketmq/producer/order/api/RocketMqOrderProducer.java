package com.gradel.parent.ali.rocketmq.producer.order.api;
...
public interface RocketMqOrderProducer extends Admin {
     ...
     */
    <T> boolean send(final RockMqOrderMessage<T> message, final MqSerializer<T> serializer);
     ...
     * @return 发送结果，true 表示发送成功，否则发送失败
     */
    <T> RockMqSendResult sendBackResult(final RockMqOrderMessage<T> message, final MqSerializer<T> serializer);
}
