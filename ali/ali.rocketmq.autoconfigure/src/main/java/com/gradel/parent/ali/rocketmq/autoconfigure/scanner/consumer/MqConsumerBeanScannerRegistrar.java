...
import com.gradel.parent.ali.rocketmq.autoconfigure.MqConsumer;
 ...
 * @date 2019/01/02 下午10:17
 */
@Deprecated
@Slf4j
public class MqConsumerBeanScannerRegistrar
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        log.debug("Searching for Mq Consumer with @MqConsumer");
        ...
        try {
            ...
            scanner.setAnnotationClass(MqConsumer.class);
//                scanner.setMarkerInterface(RocketMqMessageListener.class);
            ...
            if (CollectionUtil.isNotEmpty(groupBeanMap)) {
                registerRocketMqConsumerBeans(groupBeanMap, registry);
            }

        } catch (IllegalStateException ex) {
            log.debug("Could not determine auto-configuration package, automatic MqConsumer scanning disabled.", ex);
        }
    }

    private void registerRocketMqConsumerBeans(Map<String, List<RuntimeBeanReference>> groupBeanMap, BeanDefinitionRegistry registry) {
        for (String beanName : groupBeanMap.keySet()) {
            List<RuntimeBeanReference> subscriptionList = groupBeanMap.get(beanName);
            RootBeanDefinition beanDefinition = new RootBeanDefinition();
            beanDefinition.setBeanClass(RocketMqConsumerPostBean.class);
