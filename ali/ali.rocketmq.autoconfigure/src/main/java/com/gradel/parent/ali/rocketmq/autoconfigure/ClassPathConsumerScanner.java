...
public class ClassPathConsumerScanner extends ClassPathBeanDefinitionScanner {
    ...
    public Map<String, List<RuntimeBeanReference>> doScanAndGet(String... basePackages) {
        ...
        if (beanDefinitions.isEmpty() && existBeanNames.isEmpty()) {
            logger.warn("No " + this.annotationClass.getSimpleName() + " was found in '" + Arrays.toString(basePackages) + "' package. Please check your configuration.");
        } else {
    ...
    private void processBeanDefinitions(Map<String, Set<RuntimeBeanReference>> groupBeanMap, Set<BeanDefinitionHolder> beanDefinitions) {
        ScannedGenericBeanDefinition definition;
        for (BeanDefinitionHolder holder : beanDefinitions) {
            definition = (ScannedGenericBeanDefinition) holder.getBeanDefinition();
            AnnotationAttributes annoAttrs = AnnotationAttributes.fromMap(definition.getMetadata().getAnnotationAttributes(this.annotationClass.getName()));
            String group = annoAttrs.getString("group");
            if (StringUtil.isBlank(group)) {
                throw new IllegalArgumentException("BeanName[" + holder.getBeanName() + "] " + this.annotationClass.getSimpleName() + " group must not empty!");
            }
    ...
    private void processBeanDefinitionsWithExistBeanNames(Map<String, Set<RuntimeBeanReference>> groupBeanMap, Set<GroupBeanName> existBeanNames) {
    ...
    protected boolean checkCandidate(String beanName, BeanDefinition beanDefinition) {
        if (super.checkCandidate(beanName, beanDefinition)) {
            ...
            logger.warn("Found " + this.annotationClass.getSimpleName() + " with name '" + beanName
                    + "' and '" + beanDefinition.getBeanClassName() + "' " + this.annotationClass.getSimpleName()
