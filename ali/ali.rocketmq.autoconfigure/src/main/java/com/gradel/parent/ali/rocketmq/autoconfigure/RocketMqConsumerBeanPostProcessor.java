...
import com.gradel.parent.ali.rocketmq.autoconfigure.bean.RocketMqConsumerPostBean;
 ...
 * @date 2018/11/14 下午8:05
 */
@Slf4j
public class RocketMqConsumerBeanPostProcessor implements BeanDefinitionRegistryPostProcessor, EnvironmentAware{
    ...
    public RocketMqConsumerBeanPostProcessor(Class<? extends Annotation> annotationClass, String... packagesToScan) {
        this(annotationClass, Arrays.asList(packagesToScan));

    }

    public RocketMqConsumerBeanPostProcessor(Class<? extends Annotation> annotationClass, Collection<String> packagesToScan) {
        this(annotationClass, new LinkedHashSet<String>(packagesToScan));
    }

    public RocketMqConsumerBeanPostProcessor(Class<? extends Annotation> annotationClass, Set<String> packagesToScan) {
    ...
    private void registerServiceBeans(Set<String> packagesToScan, BeanDefinitionRegistry registry) {
        ...
        if (annotationClass.isAssignableFrom(MqConsumer.class)) {
            ...
            throw new IllegalArgumentException("annotationClass must be isAssignableFrom MqConsumer.class or MqOrderConsumer.class");
        }

        if (log.isDebugEnabled()) {
            log.debug("Searching for Mq order Consumer with @" + StringUtil.capitalize(annotationClass.getSimpleName()));
        }
        ...
        Map<String, List<RuntimeBeanReference>> pathBeanMap = scanner.doScanAndGet(StringUtils.toStringArray(packagesToScan));
        if (CollectionUtil.isNotEmpty(pathBeanMap)) {
            registerRocketMqConsumerBeans(pathBeanMap, registry, consumerBeanClazz);
        }
    }

    private void registerRocketMqConsumerBeans(Map<String, List<RuntimeBeanReference>> groupBeanMap, BeanDefinitionRegistry registry, Class<?> consumerBeanClazz) {
