 ...
 * @date 2019/01/02 下午11:15
 */
@Slf4j
public class RegUtil {

    public static void regRocketMqProducerBeans(BeanDefinitionRegistry registry, String producerPrefix, Class<?> mqProducerBanClass, Environment environment) {
        ...
        for (Map.Entry<String, Map<String, String>> entry : producers.entrySet()) {
            ...
            if (map == null || map.isEmpty()) {
                throw new IllegalArgumentException("MqProducer[" + producerPrefix + producerBeanNames + "] properties must not empty!");
            }
            ...
            if (mqProducerBanClass == RocketMqTransactionProducerBean.class && StringUtil.isBlank(transactionMessageCheckerBeanName)) {
                throw new IllegalArgumentException("MqProducer[" + producerPrefix + producerBeanNames + ".transactionMessageCheckerBeanName] properties must not empty! @see " + RocketMqTransactionMessageChecker.class);
            }
            registerRocketMqProducerBeans(mqProducerBanClass, producerBeanNames, registry, properties, transactionMessageCheckerBeanName);
        }
    }
